...
public class TicketForm {
                            ...
                            for (int j = 0; j < 8; j++) {
                                if (schedule.getReservation(i, j) != null) {
                                    if (id.equals(schedule.getReservation(i, j).getTicketID())) {
                                        selectedTicket = schedule.getReservation(i, j);
    ...
    public void hapusTiketButtonOnAction() {
        Schedule show = selectedTicket.getSchedule();
        for (int i = 0; i < 12; i++) {
            for (int j = 0; j < 8; j++) {
                if (show.getReservation(i, j) != null && show.getReservation(i, j) == selectedTicket) {
                    show.addReservation(null, i, j);
                }
            }
        }
        ...
        idTextField.setText("");
        detailsPane.setVisible(false);
        parentController.scheduleFormController.resetScheduleButtonOnAction();
    }
